...
public fun <P: Preference> PreferenceFragment.bindPreference(key: String)
        : ReadOnlyProperty<PreferenceFragment, P> = preferenceRequired(key, preferenceFinder)
public fun <P: Preference> PreferenceFragment.bindPreference(keyId: Int)
        : ReadOnlyProperty<PreferenceFragment, P> = preferenceRequired(keyId, preferenceFinderByResourceId)
...
private val PreferenceFragment.preferenceFinder: PreferenceFragment.(String) -> Preference?
    get() = { findPreference(it) }
private val PreferenceFragment.preferenceFinderByResourceId: PreferenceFragment.(Int) -> Preference?
    get() = { findPreference(resources.getString(it)) }
...
private fun preferenceNotFound(key:String, desc: KProperty<*>): Nothing =
        throw IllegalStateException("Preference Key $key for '${desc.name}' not found")

private fun preferenceNotFound(keyId:Int, desc: KProperty<*>): Nothing =
        throw IllegalStateException("Preference Key $keyId for '${desc.name}' not found")
...
@Suppress("UNCHECKED_CAST")
private fun <T, P: Preference> preferenceRequired(key: String, finder: T.(String) -> Preference?)
        = Lazy { t: T, desc -> t.finder(key) as P? ?: preferenceNotFound(key, desc) }

@Suppress("UNCHECKED_CAST")
private fun <T, P: Preference> preferenceRequired(keyId: Int, finder: T.(Int) -> Preference?)
        = Lazy { t: T, desc -> t.finder(keyId) as P? ?: preferenceNotFound(keyId, desc) }
